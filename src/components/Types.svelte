<script lang="ts">
    ...
    onMount(async () => {
        try {
            ...
            has_error = true;
            error_message =
                "Unable to connect to OpenAI API. Please check your API key and try again.";
        }
        try {
            // console.log(models);
            $types = (await apiGet(`summaryengine/v1/types`)).map(
                (type: IType) => {
                    type.ID = Number(type.ID);
                    type.openai_method =
                        String(type.openai_method) || "complete";
                    type.cut_at_paragraph = !!type.cut_at_paragraph;
                    type.openai_frequency_penalty = Number(
                        type.openai_frequency_penalty,
                    );
                    type.openai_max_tokens = Number(type.openai_max_tokens);
                    type.openai_presence_penalty = Number(
                        type.openai_presence_penalty,
                    );
    ...
    async function saveType(type: IType): Promise<void> {
        try {
            ...
            if (type.name === "New Type")
                throw "Please rename the type before saving";
            type.append_prompt = type.append_prompt || "";
            type.custom_action = type.custom_action || "";
            const result = await apiPost(
                `summaryengine/v1/type/${type.ID}`,
                type,
            );
            if (!type.ID) {
                type.ID = Number(result.id);
                $types = $types.sort((a: IType, b: IType) =>
                    a.name < b.name ? -1 : 1,
                );
                addEmptyType();
                $types = $types;
    ...
    async function deleteType(type: IType): Promise<void> {
        try {
            pending_delete = false;
            deleting = true;
            await apiDelete(`summaryengine/v1/type/${type.ID}`);
            $types = $types.filter((t: IType) => t.ID !== type.ID);
            tab = 0;
            deleting = false;
            window.scrollTo(0, 0);
        } catch (e) {
...
<nav class="nav-tab-wrapper">
    {#each $types as type, i}
        <a
            ...
            class="nav-tab"
            on:click={() => (tab = i)}
            class:nav-tab-active={tab === i}>{type.name}</a
        >
    {/each}
</nav>
                    ...
                    <tr>
                        ...
                        <td>
                            <textarea
                                name="custom_action"
                                class="regular-text"
                                bind:value={type.custom_action}
                            ></textarea>
                            <p>
                                Call a custom action based on the summary and
