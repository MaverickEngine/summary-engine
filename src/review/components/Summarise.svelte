<script>
    ...
    async function approve() {
        try {
            approving = true;
            await apiPost(`/summaryengine/v1/rate/${summary.summary_id}`, {
                rating: 1,
            });
    ...
    async function save() {
        try {
            saving = true;
            await apiPut(`/summaryengine/v1/summary/${summary.summary_id}`, {
...
<div
    class="summaryengine-summarise"
    on:mouseenter={() => (hovering = true)}
    on:mouseleave={() => (hovering = false)}
    class:is_hovering={hovering}
    class:just_summarised
    class:approved={Number(summary.summary_details.rating) === 1}
    class:unapproved={Number(summary.summary_details.rating) === 0}
    class:disapproved={Number(summary.summary_details.rating) === -1}
    role="region"
    aria-label="Summary"
>
    {#if summary.summary}
        {#if editing}
            <textarea
                class="summaryengine-summarise__summary-textarea"
                bind:value={summary.summary}
            />
            {#if !saving}
                <input
                    class="summaryengine-button button"
                    type="button"
                    name="save"
                    value="Save"
                    on:click={save}
        ...
        {#if custom_action}
            {@html custom_action
                .replace("[post_url]", post.permalink)
                .replace(
                    "[summary_encoded]",
